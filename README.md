# Лабораторная 2. Виртуальный сервер
...
- Параметр `-boot d` выполняет загрузку с CD/DVD, то есть iso образ.
...
После некоторого ожидания выбрал целый диск для пользователя - без разделения. Несколько раз подтверждаю свой выбор.

Случайно нажал Enter при выбора основных пакетов, поэтому пришлось выполнить установку заново.

В этот раз использовал команду с другими параметрами для улучшения характеристик VM:
...
- `-cpu host` VM будет использовать максимально похожий cpu (со всеми технологиями, библиотеками и т.д.) вместо дополнительного слоя эмуляции безопасной виртуальной модели cpu.  
...
- `-netdev` пробросавывает порты с хоста к гостю.

### Установка и настройка программ

Выбрав ОС из списка, немного подождав увидел терминал. Залогинился под user и сразу проверил его работоспособность:
...
- **unzip** - распаковщик архивов типа `.zip`
...
Испугавшись, что мне прийдётся переустанавливать ОС заново, я узнал, что можно спокойно поменять данный пароль через sudo - что я и сделал (пароль: `1111`).

![список скаченных архивов](./images/files_list.png)

Распаковал по своим папка по пути `/var/www/`.
...
- Перезапускаю таблицу прав, для немедленного вступления их в силу.
...
Тег **VirtualHost** объявляет новый виртуальный хост (для возможности поднятия несколько серверов).
...
Я так же попробовал подключиться к ранее созданной БД на сайте wordpress, но у меня так и не получалось. Оказалось, что у меня почему то не было прав на созданную БД от созданного пользователя. Я выдал права, и спокойно подключился на сайте к бд.
...
4. Какие преимущества, с вашей точки зрения, даёт виртуализация?
    ...
    Удобство использования: удобно создавать, удалять, менять VM - экспериментировать, или проверять совместимости программ. Это всё никак не ломает основную ОС.

5. Для чего необходимо устанавливать время / временную зону на сервере?

    Для корректной работы сервисов, использующих системное время, корректность логов по времени, согласованность работы с клиентами/серверами в других часовых поясах.  
